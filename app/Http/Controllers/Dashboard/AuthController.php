    ...
    public function index(): JsonResponse
    {
        if (request()->ajax()) {
            $products = Product::query()
                ->when(filled(request()->type), function (Builder $builder) {
                    $builder->whereType(request()->type);
                })
                ->when(filled(request()->status), function (Builder $builder) {
                    $builder->whereStatus(request()->status);
                })
                ->when(filled(request()->creationDate), function (Builder $builder) {
                    $dateRange = explode(' / ', request()->creationDate);
                    $builder
                        ->where('created_at', '>=', Carbon::parse($dateRange[0]))
                        ->where('created_at', '<=', Carbon::parse($dateRange[1])->addDay());
                })
            ...
            $dataTable = Datatables::of($products)
                ...
                ->addColumn('action', function ($row) {
                    $ShowUrl   = checkPermission('show_product') ? route('product.show', $row->id) : 0;
                    $EditUrl   = checkPermission('edit_product') ? route('product.edit', $row->id) : 0;
                    $DeleteUrl = checkPermission('delete_product') ? route('product.delete', $row->id) : 0;
                ...
                ->addColumn('creationDate', function ($row) {
                    return getDateFormat($row["created_at"]);
                })
                ->make(true);
